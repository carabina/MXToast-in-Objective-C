...
@implementation MXToastWindow

+ (instancetype)defaultWindow {
    static MXToastWindow *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[MXToastWindow alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
    });
    return instance;
}
...
- (void)bringWindowToTop:(NSNotification *)notification {
    if ([notification object] == nil || ![[notification object] isKindOfClass:[MXToastWindow class]]) {
        [[MXToastWindow defaultWindow] setHidden:YES];
        [[MXToastWindow defaultWindow] setHidden:NO];
    }
}
...
- (void)handleRotate:(UIInterfaceOrientation)orientation {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        MXToast *toast = [[MXToastManager defaultManager] currentToast];
        if (toast != nil) {
